...
const VisualizationArea = ({ bitSort, animation, showBinary, theme }) => {
  const { currentArray, currentBitPosition, maxBits } = bitSort;

  return (
    <div
      className={`lg:col-span-2 ${theme.cardClass} rounded-lg p-4 shadow flex flex-col`}
    >
      <div className="flex-1 space-y-4 mb-4 max-h-96 overflow-y-auto">
        {currentArray.map((number, index) => (
          <div
            key={index}
            className={`flex items-center gap-4 p-3 rounded-lg ${theme.secondaryBgClass}`}
          >
                  ...
                  .map((bit, bitIndex) => (
                    <div
                      ...
                      className={`w-8 h-8 flex items-center justify-center rounded font-bold text-sm border-2 transition-all ${
                        maxBits - bitIndex - 1 === currentBitPosition
      ...
      <Controls bitSort={bitSort} animation={animation} theme={theme} />
    </div>
  );
};
